...
static int   s_rotation;            /* 0 none, 1 ROT90 CW, 2 ROT270 CCW  */
...
/* Rotate a device-space delta into render space.
 *
 *   ROT90 CW  (1): (dgx,dgy) = ( dpy, -dpx)
 *   ROT270 CCW(2): (dgx,dgy) = (-dpy,  dpx)
 *   none      (0): unchanged
 *
 * `rot` is passed per-device, and that distinction matters: a controller that
 * physically turns with the screen (the attached handheld one) needs the
 * display rotation, while detached Joy-Cons, a docked pad, and a USB mouse --
 * a desk device that never turns with the console -- must NOT be rotated.
 * Rotating the mouse is the classic mistake here; it feels broken immediately. */
...
/* Rotation for STICK/GYRO: only when the display is rotated AND the input came
 * from the console's built-in controller, which turns with the screen. */
...
static void do_touch(void) {
  ...
  /* panel(px,py) landscape -> render(x,y). Under rotation the render axes swap,
   * so each render axis is scaled by the panel axis that actually feeds it.
   * The touch panel is bonded to the physical glass and does not rotate just
   * because the image does -- this is what reconciles the two. */
...
static void do_gyro(void) {
  ...
  if (fabsf(dx) > 0.05f || fabsf(dy) > 0.05f) {
    float rdx, rdy;
    nxp_rot_delta(nxp_ctrl_rot(), dx, dy, &rdx, &rdy);
    s_cx += rdx;
    s_cy += rdy;
    clamp_cursor();
  }
...
static int do_mouse(void) {
  ...
  if (dx || dy) {
    /* Deliberately NOT rotated: a USB mouse sits on a desk and does not turn
     * with the console, so its axes already match what the player sees. */
...
void nxp_update(void) {
  ...
  if (s_visible > 0) {
    ...
    if (ls.x || ls.y) {
      /* Rotated only when the pad turns with the screen -- see nxp_ctrl_rot. */
      const float sdx =  (ls.x / 32767.0f) * s_stick_speed;
      const float sdy = -(ls.y / 32767.0f) * s_stick_speed;   /* stick +y is up */
      float rdx, rdy;
      nxp_rot_delta(nxp_ctrl_rot(), sdx, sdy, &rdx, &rdy);
      s_cx += rdx;
      s_cy += rdy;
      clamp_cursor();
    }
