...
void so_finalize(so_module *mod) {
  ...
  // svcMapProcessCodeMemory revokes all access to the SOURCE range, so every
  // load_base-relative pointer we kept is now a fault waiting to happen.
  // mod->syms and mod->dynstrtab were computed against load_base in so_load,
  // and .dynsym/.dynstr live inside the first PT_LOAD, which is mapped and
  // readable at load_virtbase -- so rebase them instead of losing them.
  //
  // Without this, any symbol lookup after so_finalize() (so_try_find_addr_rx,
  // and therefore so_find_addr_in_loaded and the dlsym shim) reads through a
  // revoked mapping and takes a Data Abort.
  if (mod->syms)
    mod->syms = (Elf64_Sym *)((uintptr_t)mod->load_virtbase +
                              ((uintptr_t)mod->syms - (uintptr_t)mod->load_base));
  if (mod->dynstrtab)
    mod->dynstrtab = (char *)((uintptr_t)mod->load_virtbase +
...
uintptr_t so_try_find_addr_rx(so_module *mod, const char *symbol) {
  if (!mod || !mod->syms || !mod->dynstrtab || mod->num_syms <= 0)
    return 0;
  for (int i = 0; i < mod->num_syms; i++) {
    char *name = mod->dynstrtab + mod->syms[i].st_name;
    if (strcmp(name, symbol) == 0)
...
// search every currently-loaded module for an exported symbol (used by the
// dlsym shim as a last resort). returns 0 if no module exports it.
uintptr_t so_find_addr_in_loaded(const char *symbol) {
  for (so_module *m = so_list; m; m = m->next) {
    const uintptr_t addr = so_try_find_addr_rx(m, symbol);
    if (addr)
      return addr;
  }
  return 0;
