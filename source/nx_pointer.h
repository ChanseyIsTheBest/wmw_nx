...
typedef struct {
  ...
  /* Screen rotation, for ports whose framebuffer is rotated (TATE).
   * The cursor stays in RENDER space -- nxp_draw() runs before the rotation
   * blit, so it is carried by it -- but physical device motion is rotated to
   * match: 0 = none, 1 = render rotated 90 CW, 2 = render rotated 90 CCW. */
  int   rotation;

  /* If 0, this module does NOT read the touchscreen (the host handles touch to
   * keep its own multitouch path). Nonzero => module reads touch.            */
  int   handle_touch;
